...
namespace FashionShop.Application.Services.ProductServices
{
    public class ProductService : IProductService
    {
        ...
        public async Task<Result<ProductResponseDTO>> CreateProductAsync(CreateProductDTO productDto, CancellationToken cancellationToken)
        {
            ...
            if (productDto.ThumbnailFile != null)
                thumbnailUrl = await _photoService.UploadPhotoAsync(productDto.ThumbnailFile, "products");
        ...
        public async Task<Result> DeleteProductAsync(Guid id, CancellationToken cancellationToken)
        {
            ...
            if (!string.IsNullOrWhiteSpace(product.ThumbnailUrl))
                await _photoService.DeletePhotoAsync(product.ThumbnailUrl);
        ...
        public async Task<Result<PageResponse<ProductResponseDTO>>> GetProductsByCategoryAsync(int pageIndex, int pageSize, Guid categoryId, CancellationToken cancellationToken)
        {
            try
            {
                var filterSpecification = new ProductFilterSpecification(null, categoryId, pageIndex: pageIndex, pageSize: pageSize);
                var root = await _productRepository.PagedListAsync(filterSpecification, pageIndex, pageSize, cancellationToken);

                var itemOnPage = root.Items.Select(MapProductToProductResponseDTO).ToList();
                var response = new PageResponse<ProductResponseDTO>(itemOnPage, root.Total, pageSize, pageIndex);
        ...
        public async Task<Result<PageResponse<ProductResponseDTO>>> GetAllProductAsync(int pageIndex, int pageSize, CancellationToken cancellationToken)
        {
            try
            {
                ...
                var products = root.Items.Select(MapProductToProductResponseDTO).ToList();
        ...
        public async Task<Result<ProductResponseDTO>> UpdateProductAsync(Guid id, UpdateDetailsProductDTO productDto, CancellationToken cancellationToken)
        {
            ...
            var nextThumbnailUrl = product.ThumbnailUrl;

            if (productDto.ThumbnailFile is { Length: > 0 })
            {
                var uploadedThumbnailUrl = await _photoService.UploadPhotoAsync(productDto.ThumbnailFile, "products");
            ...
            product.UpdateDetails(productDto.Name, productDto.Description, nextThumbnailUrl);
